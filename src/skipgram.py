...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        noise_dist, num_samples=batch_size * n_samples, replacement=True
    )
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        out_loss = torch.nn.functional.logsigmoid(pos_dot_product)

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        neg_dot_product = torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze(2)
        noise_loss = torch.nn.functional.logsigmoid(-neg_dot_product).sum(dim=1)
